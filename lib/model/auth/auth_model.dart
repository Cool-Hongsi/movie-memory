...
class AuthModel with ChangeNotifier {
  ...
  Future<void> authGoogleSignIn() async {
    ...
    try{
      ...
      docRef.get().then((data) {
        if(data.exists){
          // Aleady Exist
        } else { // If it is first time to sign in by google
  ...
  Future<dynamic> authGetCurrentUserData() async {
    try{
      ...
      DocumentSnapshot currentUser = await _store.collection('users').document(currentUserFirebaseData.uid).get();
  ...
  Future<String> authProfileEdit(File pickedImage, String userName) async {
    ...
    DocumentSnapshot currentUserSnapShot = await _store.collection('users').document(currentUser.uid).get();

    if(pickedImage != null && userName == null) { // Only Edit Image
      try{
        // Check whether it has already user_image url
        if(currentUserSnapShot.data['user_image'] != null) { // image already has
          final fileUrl = Uri.decodeFull(path.basename(currentUserSnapShot.data['user_image'])).replaceAll(new RegExp(r'(\?alt).*'), '');
          final refDelete = _storage.ref().child(fileUrl);
          await refDelete.delete();

          final refAdd = _storage.ref().child('users').child(currentUser.uid).child(pickedImage.path);
    ...
    } else if(pickedImage == null && userName != null) { // Only Edit Username
      try{
        await _store.collection('users').document(currentUser.uid).updateData({
          'edited_at': Timestamp.now(),
          'username': userName
    ...
    } else if(pickedImage != null && userName != null) { // Edit Both Image and Username
      try{
        // Check whether it has already user_image url
        if(currentUserSnapShot.data['user_image'] != null) { // image already has
          final fileUrl = Uri.decodeFull(path.basename(currentUserSnapShot.data['user_image'])).replaceAll(new RegExp(r'(\?alt).*'), '');
  ...
  Future<String> deleteAccount(String userPassword, String feedback) async {
    ...
    try {
      ...
      await _store.collection('users').document(currentUser.uid).get().then((userData) {
        signInType = userData.data['sign_in_with_google'];

        if(userData.data['user_image'] != null) { // Exist profile image in storage
          final fileUrl = Uri.decodeFull(path.basename(userData.data['user_image'])).replaceAll(new RegExp(r'(\?alt).*'), '');
          final refDelete = _storage.ref().child(fileUrl);
      ...
      if(signInType) { // google sign in
        await _store.collection('users').document(currentUser.uid).get().then((userData) {
          credential = GoogleAuthProvider.getCredential(idToken: userData.data['idToken'], accessToken: userData.data['accessToken']);
        });
      } else { // general sign in
        credential = EmailAuthProvider.getCredential(email: currentUser.email, password: userPassword);
      }
